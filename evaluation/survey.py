...
class survey_question(osv.Model):
    ...
    def _get_max_score(self):
        # TODO mejorar esto y ver porque se llama varias
        # veces a esta funcion *hay que cambiarlo tambien en
        #  academic_reports que sobreescribimos esta funcion
        max_score = 0
        question = self
    ...
    max_score = new_fields.Integer(compute='_get_max_score',
                                   string='Max Score',
                                   help='Max score an answer of '
                                        'this question can get',
                                   store=True)

    _columns = {
        'objective_id': fields.many2one('survey.question.objective',
                                        string='Objective',),
        'level_id': fields.many2one('survey.question.level',
                                    string='Level', ),
...
class survey_user_input(osv.Model):
    ...
    _columns = {
        ...
        'input_question_score_ids': fields.one2many(
            ...
            'user_input_id', string='Question Scores',
            readonly=True,),
        # 'manual_score': fields.related(
        # 'survey_id', 'manual_score',
        # string="Survey Manual Score?",
        # type="boolean", readonly=True),
        # 'without_questions': fields.related('survey_id',
        # 'without_questions', string="Survey Without Questions?",
        # type="boolean", readonly=True),
        'evaluation_type': fields.related(
            'survey_id', 'evaluation_type',
            string="Evaluation Type", type="char",
            readonly=True),
        # This one was replaced by without_questions functionality
        # 'designed': fields.related('survey_id',
        # 'designed', string="Survey Is designed?",
        # type="boolean", readonly=True),
    }
        ...
        for user_input in self.browse(cr, uid, ids, context=context):
            ...
            question_ids = question_obj.search(
                ...
                [('survey_id', '=', user_input.survey_id.id),
                 ('max_score', '!=', 0)])
            for question in question_obj.browse(
                ...
                if question.type == 'simple_choice' \
                    ...
                    pre_score = sum([self.get_answer_score(user_input_line)
                                     for user_input_line in user_input_lines])
                    question_score = self.get_ranged_score(
                        cr, uid, question, pre_score, context=context)
    ...
    def get_ranged_score(
        ...
        question_score_range_ids = \
            question_score_range_obj.search(
                cr, uid, [('from', '<=', pre_score),
                          ('to', '>=', pre_score),
        ...
        if not question_score_range_ids:
            ...
            return question_score_range_obj.browse(
...
class survey_matrix_answer_score(osv.Model):
    ...
    _columns = {
        'score': fields.integer('Score',
                                required=True,),
        'question_id': fields.many2one('survey.label',
                                       'Question'),
        'answer_id': fields.many2one('survey.label',
                                     'Answer',
                                     required=True,),
    }
...
class survey_level(osv.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        res = self._complete_name(
            cr, uid, ids,
            'complete_name', None, context=context)
        return res.items()

    def _complete_name(
            self, cr, uid, ids, name,
            args, context=None):
        """ Forms complete name of location
        from parent location to child location.
    ...
    def _get_score(self, cr, uid, ids, name, args, context=None):
        ...
        for level in self.browse(cr, uid, ids, context=context):
            ret[level.id] = \
                sum([question.max_score for question in
                     level.question_ids])
        return ret
    ...
    _columns = {
        ...
        'score': fields.function(_get_score,
                                 type='integer', string='Score',
                                 readonly=True,),
...
class survey_content(osv.Model):
    ...
    def _complete_name(self, cr, uid, ids, name,
                       args, context=None):
        """ Forms complete name of location
        from parent location to child location.
        @return: Dictionary of values
    ...
    def _get_score(self, cr, uid, ids, name,
                   args, context=None):
        ret = dict()
        for level in self.browse(cr, uid, ids, context=context):
            ret[level.id] = sum([question.max_score
    ...
    _columns = {
        'content_id': fields.many2one('survey.question.content',
                                      ...
                                      translate=True),
        'score': fields.function(_get_score, type='integer',
                                 string='Score', readonly=True,),
        'survey_id': fields.many2one('survey.survey',
                                     string='Survey', required=True,),
    }
...
class survey_objective(osv.Model):
    ...
    def name_get(self, cr, uid, ids, context=None):
        # always return the full hierarchical name
        res = self._complete_name(cr, uid, ids, 'complete_name',
                                  None, context=context)
        return res.items()

    def _complete_name(self, cr, uid, ids, name, args, context=None):
        """ Forms complete name of location from parent
        location to child location.
        @return: Dictionary of values
        """
        res = {}
        for line in self.browse(cr, uid, ids):
    ...
    def _get_score(self, cr, uid, ids, name, args, context=None):
        ...
        for objective in self.browse(cr, uid, ids, context=context):
            ret[objective.id] = sum([question.max_score for
                                     question in objective.question_ids])
        return ret

    _columns = {
        'objective_id': fields.many2one('survey.question.objective',
                                        string="Objective", required=True,
                                        translate=True),
        'score': fields.function(_get_score, type='integer',
                                 string='Score', readonly=True,),
...
class survey_survey(osv.Model):
    ...
    def _get_scores(self, cr, uid, ids, names, arg, context=None):
        ...
        for survey in self.browse(cr, uid, ids, context=context):
            ...
            obj_questions_score = sum([objective.score for objective in
                                       survey.question_objective_ids])
            res[survey.id]['obj_questions_score'] = obj_questions_score
            non_obj_questions_ids = self.pool.get('survey.question').search(
                cr, uid, [('survey_id', '=', survey.id),
                          ('objective_id', '=', False)], context=context)
            non_obj_questions = self.pool.get('survey.question').browse(
                cr, uid, non_obj_questions_ids, context=context)
            ...
            res[survey.id]['non_level_questions_score'] = \
                non_level_questions_score

            res[survey.id]['max_score'] = \
                obj_questions_score + non_obj_questions_score
        return res
    ...
    _columns = {
        ...
        'evaluation_type': fields.selection(
            ...
            help="-Automattically Evaluated:"
            " you have to set espected "
            "question results and the evaluation"
            " will be evaluated automatically."
            " \n-Manually Evaluated: someone will "
            "need to correct the evaluation and "
        ...
        #     string="Evaluation Type",
        #     help="-Automattically Evaluated:
        # you have to set espected question results
        # and the evaluation will be evaluated automatically.
        #         \n-Manually Evaluated: someone will
        ...
        'question_level_ids': fields.one2many('survey.level',
                                              ...
                                              copy=True),
        'question_content_ids': fields.one2many('survey.content',
                                                'survey_id',
                                                string='Questions Contents',
                                                copy=True),
        'obj_questions_score': fields.function(_get_scores,
                                               type='integer',
                                               string='Objetives Score',
                                               help='Score for questions '
                                                    'with objetive defined',
                                               multi='_get_scores', ),
        'non_obj_questions_score': fields.function(
            _get_scores, type='integer',
            string='Other Questions Score',
            help='Score for questions without '
                 'objetive defined',
            multi='_get_scores', ),
        'content_questions_score': fields.function(
            _get_scores, type='integer',
            string='Contents Score',
            help='Score for questions with content defined',
            multi='_get_scores', ),
        'non_content_questions_score': fields.function(
            _get_scores, type='integer',
            string='Other Questions Score',
            help='Score for questions without content defined',
            multi='_get_scores', ),
        'level_questions_score': fields.function(
            _get_scores, type='integer', string='Levels Score',
            help='Score for questions with level defined',
            multi='_get_scores', ),
        'non_level_questions_score': fields.function(
            _get_scores, type='integer',
            string='Other Questions Score',
            help='Score for questions without level defined',
            multi='_get_scores', ),
        'max_score': fields.function(
            _get_scores, type='integer',
            string='Max Score',
            help='Maximum score that can be obtained in '
                 'this evaluation', multi='_get_scores', ),
    }
    ...
    def compute_score(self, cr, uid, ids, context=None):
        ...
        total_user_input_ids = user_input_obj.search(
            cr, uid, [('survey_id', 'in', ids)],
            order='id', context=context)
        user_input_obj.compute_score(cr, uid, total_user_input_ids,
                                     context=context)
